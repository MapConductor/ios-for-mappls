...
@MainActor
final class MapplsCircleController: CircleController<MGLPolygonFeature, MapplsCircleOverlayRenderer> {
    private weak var mapView: MGLMapView?

    init(mapView: MGLMapView?) {
        self.mapView = mapView

        let circleManager = CircleManager<MGLPolygonFeature>()
        ...
        let renderer = MapplsCircleOverlayRenderer(
            mapView: mapView,
