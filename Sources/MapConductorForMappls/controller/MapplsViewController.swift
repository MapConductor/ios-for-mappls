...
final class MapplsViewController: MapViewControllerProtocol {
    let holder: AnyMapViewHolder
    let typedHolder: MapplsMapViewHolder
    ...
    /// パン範囲の制限に使う。
    ///
    /// Mappls iOS は MapLibre ベースなので制限も MapLibre と同じ 2 系統になる:
    ///
    /// - **ジェスチャー**: `MGLMapViewDelegate` の
    ...
    init(mapView: MGLMapView) {
        ...
        let typedHolder = MapplsMapViewHolder(mapView: mapView)
    ...
    func moveCamera(position: MapCameraPosition) {
        ...
        let cameraState = position.toMapplsCameraState()

        // ★ pitch を先に、zoom を後に当てる。順序を逆にすると**傾けるたびに地図が遠ざかる**。
        //
        // `MGLMapCamera` が持っているのは真下方向の距離 `altitude` で、実際の縮尺を決めるのは
        ...
        //
        // Note: MGLMapView.camera is a copy; mutating mapView.camera.pitch does not affect the map.
    ...
    func fitBounds(bounds: GeoRectBounds, padding: Int) {
        ...
        let coordinateBounds = MGLCoordinateBoundsMake(
...
private final class CameraAnimator {
    ...
    /// 中心とズームの補間。van Wijk（＝ android-for-mappls の `map.animateCamera` が落ちる `NativeMap.flyTo` と同じ式）。
    ...
    @objc private func step(_ displayLink: CADisplayLink) {
        ...
        let cameraState = currentPos.toMapplsCameraState()
