...
@MainActor
final class MapplsGroundImageOverlayRenderer: AbstractGroundImageOverlayRenderer<MapplsGroundImageHandle> {
    ...
    func createGroundImageSync(state: GroundImageState) -> MapplsGroundImageHandle? {
        ...
        let imageSource = MGLImageSource(identifier: sourceId, coordinateQuad: coordinates, image: state.image)
        let layer = MGLRasterStyleLayer(identifier: layerId, source: imageSource)
        ...
        return MapplsGroundImageHandle(
            sourceId: sourceId,
            layerId: layerId,
    ...
    func updateGroundImageSync(
        groundImage: MapplsGroundImageHandle,
        current: GroundImageEntity<MapplsGroundImageHandle>,
        prev: GroundImageEntity<MapplsGroundImageHandle>
    ) -> MapplsGroundImageHandle? {
        guard let style else { return groundImage }

        guard let imageSource = style.source(withIdentifier: groundImage.sourceId) as? MGLImageSource,
              let layer = style.layer(withIdentifier: groundImage.layerId) as? MGLRasterStyleLayer else {
    ...
    func removeGroundImageSync(entity: GroundImageEntity<MapplsGroundImageHandle>) {
    ...
    override func createGroundImage(state: GroundImageState) async -> MapplsGroundImageHandle? {
    ...
    override func updateGroundImageProperties(
        ...
        current: GroundImageEntity<MapplsGroundImageHandle>,
        prev: GroundImageEntity<MapplsGroundImageHandle>
    ) async -> MapplsGroundImageHandle? {
        updateGroundImageSync(groundImage: groundImage, current: current, prev: prev)
    }

    override func removeGroundImage(entity: GroundImageEntity<MapplsGroundImageHandle>) async {
        removeGroundImageSync(entity: entity)
    }

    private func removeSourceAndLayerIfExists(style: MGLStyle, sourceId: String, layerId: String) {
    ...
    private func insertLayer(_ layer: MGLRasterStyleLayer, into style: MGLStyle) {
    ...
    private func findBelowLayer(style: MGLStyle) -> MGLStyleLayer? {
        let prefixes = [
...
private extension GeoRectBounds {
    func toCoordinateQuad() -> MGLCoordinateQuad? {
        guard let sw = southWest, let ne = northEast else { return nil }
        return MGLCoordinateQuadMake(
