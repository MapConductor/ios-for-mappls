...
@MainActor
final class MapplsMarkerRenderer: MarkerOverlayRendererProtocol {
    typealias ActualMarker = MGLPointFeature
    ...
    private let markerManager: MarkerManager<MGLPointFeature>
    ...
    init(
        mapView: MGLMapView?,
        markerManager: MarkerManager<MGLPointFeature>,
    ...
    func onStyleLoaded(_ style: MGLStyle) {
        ...
        MCLog.marker("MapplsMarkerRenderer.onStyleLoaded")
    ...
    func onAdd(data: [MarkerOverlayAddParams]) async -> [MGLPointFeature?] {
        guard let style else { return [] }
        ensureDefaultIcon(style: style)
        MCLog.marker("MapplsMarkerRenderer.onAdd count=\(data.count)")
    ...
    func onChange(data: [MarkerOverlayChangeParams<MGLPointFeature>]) async -> [MGLPointFeature?] {
        guard let style else { return [] }
        ensureDefaultIcon(style: style)
        if !data.isEmpty { MCLog.marker("MapplsMarkerRenderer.onChange count=\(data.count)") }
    ...
    func onRemove(data: [MarkerEntity<MGLPointFeature>]) async {
        for entity in data {
    ...
    func onAnimate(entity: MarkerEntity<MGLPointFeature>) async {
    ...
    private func animateMarkerDrop(entity: MarkerEntity<MGLPointFeature>, duration: CFTimeInterval) async {
        await animateMarker(entity: entity, animation: .Drop, duration: duration)
    }

    private func animateMarkerBounce(entity: MarkerEntity<MGLPointFeature>, duration: CFTimeInterval) async {
        await animateMarker(entity: entity, animation: .Bounce, duration: duration)
    }

    private func animateMarker(
        entity: MarkerEntity<MGLPointFeature>,
        ...
        MCLog.marker("MapplsMarkerRenderer.onAnimate start id=\(entity.state.id) anim=\(animation) bounds=\(String(describing: mapView.bounds))")
        mapView.layoutIfNeeded()
        if mapView.window == nil || mapView.bounds.isEmpty {
            MCLog.marker("MapplsMarkerRenderer.onAnimate defer id=\(entity.state.id) reason=windowOrBounds")
        ...
        MCLog.marker(
            "MapplsMarkerRenderer.onAnimate id=\(entity.state.id) targetPoint=\(String(describing: targetPoint)) startPoint=\(String(describing: startPoint)) startBackPoint=\(String(describing: startBackPoint))"
        )

        if !targetPoint.x.isFinite || !targetPoint.y.isFinite || !startBackPoint.x.isFinite || !startBackPoint.y.isFinite {
            MCLog.marker("MapplsMarkerRenderer.onAnimate defer id=\(entity.state.id) reason=nonFiniteProjection")
            await deferAnimate(entity: entity)
            return
        }
        ...
        if deltaX > 4.0 || deltaY > 4.0 {
            MCLog.marker("MapplsMarkerRenderer.onAnimate defer id=\(entity.state.id) reason=projectionMismatch dx=\(deltaX) dy=\(deltaY)")
            await deferAnimate(entity: entity)
            return
        }
        ...
        if targetPoint.y > 1,
           ...
           abs(startCoord.longitude - target.longitude) < 1e-10 {
            MCLog.marker("MapplsMarkerRenderer.onAnimate defer id=\(entity.state.id) reason=projectionNotReady")
            await deferAnimate(entity: entity)
            return
        }
        ...
        let runner = MarkerAnimationRunner(
            ...
            onCompletion: { [weak self] in
                ...
                MCLog.marker("MapplsMarkerRenderer.onAnimate end id=\(entity.state.id)")
                self?.animateEndListener?(entity.state)
                Task { await self?.onPostProcess() }
            }
        )
    ...
    private func deferAnimate(entity: MarkerEntity<MGLPointFeature>) async {
        let id = entity.state.id
        let attempts = (deferredAnimateAttemptsById[id] ?? 0) + 1
        deferredAnimateAttemptsById[id] = attempts
        MCLog.marker("MapplsMarkerRenderer.deferAnimate id=\(id) attempt=\(attempts)")
        guard attempts <= 20 else {
            deferredAnimateAttemptsById.removeValue(forKey: id)
            MCLog.marker("MapplsMarkerRenderer.deferAnimate id=\(id) givingUp")
    ...
    private func applyImmediatePosition(
        for entity: MarkerEntity<MGLPointFeature>,
        to marker: MGLPointFeature
    ) async {
        entity.state.animate(nil)
    ...
    private func ensureDefaultIcon(style: MGLStyle) {
    ...
    private func setImage(_ image: UIImage, name: String, style: MGLStyle) {
        // Mappls treats style images as if they were @1x. To keep icons crisp on Retina
    ...
    private func iconOffset(_ icon: BitmapIcon) -> [NSNumber] {
        // Mappls style values must be JSON-serializable. `CGVector` / `NSValue` cannot be
    ...
    private func bouncePath(for mapView: MGLMapView, target: CLLocationCoordinate2D) -> [GeoPoint] {
        let targetPoint = mapView.convert(target, toPointTo: mapView)
    ...
    private func geoPoint(for point: CGPoint, mapView: MGLMapView) -> GeoPoint {
