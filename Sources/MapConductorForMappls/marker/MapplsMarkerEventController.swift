...
/// **Mappls 固有の面の橋渡し**だけ。
///
/// 移行前はこのファイルが 84 行あり、ios-for-maplibre と ios-for-mappls で
/// **import 文以外 1 文字も違わなかった**。
@MainActor
final class MapplsMarkerEventController: DefaultMarkerEventController {
    init(mapView: MGLMapView?, markerController: MapplsMarkerController) {
        super.init(
            surface: mapView.map { MGLMarkerDragSurface(mapView: $0) },
            host: MapplsMarkerEventHost(markerController: markerController)
        )
    }
...
/// `MGLMapView` をコアの ``MarkerDragSurface`` に適合させる。
@MainActor
private final class MGLMarkerDragSurface: MarkerDragSurface {
    private weak var mapView: MGLMapView?

    init(mapView: MGLMapView) { self.mapView = mapView }
...
/// MapplsMarkerController をコアの ``MarkerEventHostProtocol`` に適合させる。
@MainActor
private final class MapplsMarkerEventHost: MarkerEventHostProtocol {
    /// **weak で持つこと。** MapplsMarkerController が eventController を強参照しており、
    /// ここを強参照にすると循環して地図を閉じても解放されない。
    private weak var markerController: MapplsMarkerController?

    init(markerController: MapplsMarkerController) { self.markerController = markerController }
