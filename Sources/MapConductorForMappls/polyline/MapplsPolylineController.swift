...
@MainActor
final class MapplsPolylineController: PolylineController<[MGLPolylineFeature], MapplsPolylineOverlayRenderer> {
    private weak var mapView: MGLMapView?

    init(mapView: MGLMapView?) {
        self.mapView = mapView

        let polylineManager = PolylineManager<[MGLPolylineFeature]>()
        ...
        let renderer = MapplsPolylineOverlayRenderer(
            mapView: mapView,
